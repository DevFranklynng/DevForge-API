import type { AiContext, AiProvider, AiRequest, AiResponse } from "../types.js";
...
function listTaskBullets(ctx: AiContext) {
...
function attentionProjects(ctx: AiContext) {
...
function behindSchedule(ctx: AiContext) {
  return ctx.projects
    .map((p) => `- **${p.name}** · progress ${p.progress}% · ${p.openTasks} open / ${p.totalTasks} tasks`)
    .sort((a, b) => a.localeCompare(b))
    .join("\n");
}

/**
 * Data-driven demo engine. Answers are generated from the user's real
 * workspace data so responses are always meaningful. Every response carries
 * `isDemo: true` and the UI displays a "Demo mode" badge.
 */
export class DemoProvider implements AiProvider {
  ...
  async ask(request: AiRequest, ctx: AiContext): Promise<AiResponse> {
    const text = request.message.toLowerCase();
    const base: Omit<AiResponse, "content"> = {
      isDemo: true,
