...
export async function exchangeCode(code, state) {
  ...
  const tokenRes = await fetchJson(GITHUB_OAUTH_TOKEN, {
    method: "POST",
...
export async function disconnectCredential(userId) {
...
async function revokeToken(encryptedToken) {
  let token;
  try {
...
export async function syncGitHubRepo(input, token) {
  ...
  try {
    const data = await fetchJson(`${GITHUB_API}/repos/${input.owner}/${input.name}`, {
      headers: {
...
export async function listUserRepositories(userId) {
  const token = await getUserToken(userId);
  if (!token) {
    throw new Error("GitHub account is not connected");
  }

  const repos = await fetchJson(`${GITHUB_API}/user/repos?per_page=100&sort=updated`, {
    headers: {
